...
export default class Db {
    ...
    //TO DO: CHECK ALL THE VIEW FUNCTIONS and then correct the queries
    // THEN UPDATE THE INDEX.TS FILE TO CALL THESE FUNCTIONS make sure the arguments are taken in correctly
    async viewAllDepartments() {
        return this.query(`SELECT * FROM departments;`);
    }
    async viewAllRoles() {
        return this.query(`SELECT roles.id, roles.title, departments.name AS department, roles.salary
                FROM roles
                LEFT JOIN departments ON roles.department_id = departments.id;`);
    }
    async viewAllEmployees() {
        return this.query(`SELECT employees.id, employees.first_name, employees.last_name, roles.title, departments.name AS department, roles.salary, 
    ...
    async addRole(role, departmentId) {
